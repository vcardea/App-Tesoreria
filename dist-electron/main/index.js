...
function getSituazioneGlobale() {
  ...
  const usciteCompletati = db.prepare(
  ...
  const accontiAperti = db.prepare(
    `SELECT SUM(acconto_fornitore) as total FROM acquisti WHERE completato = 0`
  ).get();
  const totaleUscite = (usciteCompletati.total || 0) + (accontiAperti.total || 0);
...
function addMovimentoFondo(i, d) {
  return db.prepare("INSERT INTO fondo_cassa (importo, descrizione) VALUES (?, ?)").run(i, d);
}
function getMovimentiFondo() {
...
function updateAcquisto(id, n, p, acconto) {
  return db.prepare(
...
function getAcquisti() {
  return db.prepare(`SELECT * FROM acquisti ORDER BY data_creazione DESC`).all();
}
...
function getWorkbook(filePath) {
  try {
    ...
    return readSync(fileBuffer, { type: "buffer", cellDates: true });
  } catch (error) {
...
async function parseMembersList(filePath) {
  try {
    const workbook = getWorkbook(filePath);
    const worksheet = workbook.Sheets[workbook.SheetNames[0]];
    const rows = utils.sheet_to_json(worksheet, {
...
function getPreloadPath() {
  return path.join(
    __dirname,
    process.env.VITE_DEV_SERVER_URL ? "../preload/preload.js" : "../preload/preload.js"
  );
}
function createSplashWindow() {
  ...
  const splashPath = process.env.VITE_DEV_SERVER_URL ? path.join(process.cwd(), "public/splash.html") : path.join(__dirname, "../../dist/splash.html");
...
function createMainWindow() {
  ...
  electron.ipcMain.handle(
    "add-movimento-fondo",
    (e, data) => addMovimentoFondo(data.importo, data.descrizione)
  );
  ...
  electron.ipcMain.handle(
    "update-membro",
    (e, { id, membro }) => updateMembro(id, membro)
  );
  electron.ipcMain.handle("delete-membro", (e, id) => deleteMembro(id));
  electron.ipcMain.handle("delete-all-membri", () => deleteAllMembri());
  electron.ipcMain.handle(
    "create-acquisto",
    (e, d) => createAcquisto(d.nome, d.prezzo, d.acconto)
  );
  electron.ipcMain.handle(
    "update-acquisto",
    (e, d) => updateAcquisto(d.id, d.nome, d.prezzo, d.acconto)
  );
  electron.ipcMain.handle("delete-acquisto", (e, id) => deleteAcquisto(id));
  electron.ipcMain.handle("get-acquisti", () => getAcquisti());
  electron.ipcMain.handle("get-quote", (e, id) => getQuoteAcquisto(id));
  electron.ipcMain.handle(
    "update-quota",
    (e, { id, qta, versato }) => updateQuota(id, qta, versato)
  );
  electron.ipcMain.handle("completa-acquisto", (e, id) => setAcquistoCompletato(id));
  electron.ipcMain.handle("log-ui-action", (e, msg) => logSystem("ACTION", msg));
  ...
  electron.ipcMain.handle("export-debtors", async (e, { acquistoNome, debtors }) => {
    ...
    try {
      ...
      const ws = utils.json_to_sheet(debtors);
      ws["!cols"] = [
        { wch: 20 },
        { wch: 20 },
        { wch: 10 },
...
Chiudi il file Excel "${path.basename(
  ...
  electron.ipcMain.handle("import-membri-excel", async (e, p) => {
...
electron.app.whenReady().then(setupApp);
