...
const logPath = path$1.join(electron.app.getPath("userData"), "system.log");
...
function logSystem(level, message, data) {
  ...
  logLine += "\n";
  try {
    fs$1.appendFileSync(logPath, logLine);
...
const dbPath = path$1.join(userDataPath, "tesoriere.db");
const backupDir = path$1.join(userDataPath, "backups");
...
function performBackup() {
  ...
  try {
    ...
    const backupPath = path$1.join(backupDir, backupName);
    fs$1.copyFileSync(dbPath, backupPath);
...
function cleanOldBackups() {
  try {
    const files = fs$1.readdirSync(backupDir).filter((f) => f.startsWith("backup_") && f.endsWith(".db"));
    files.sort().reverse();
    if (files.length > 10) {
      const toDelete = files.slice(10);
      toDelete.forEach((f) => {
        fs$1.unlinkSync(path$1.join(backupDir, f));
...
function getBackupsList() {
  try {
    return fs$1.readdirSync(backupDir).filter((f) => f.endsWith(".db")).sort().reverse().map((f) => {
      const stat = fs$1.statSync(path$1.join(backupDir, f));
...
function restoreBackup(filename) {
  try {
    ...
    const source = path$1.join(backupDir, filename);
    if (!fs$1.existsSync(source)) {
      logSystem("ERROR", "File backup non trovato su disco");
      return false;
    }
    try {
      if (fs$1.existsSync(dbPath)) fs$1.unlinkSync(dbPath);
      if (fs$1.existsSync(dbPath + "-wal")) fs$1.unlinkSync(dbPath + "-wal");
      if (fs$1.existsSync(dbPath + "-shm")) fs$1.unlinkSync(dbPath + "-shm");
    } catch (err) {
      logSystem("ERROR", `Impossibile pulire vecchi file DB: ${err.message}`);
    }
    fs$1.copyFileSync(source, dbPath);
...
async function parseBankStatement(filePath, membri) {
  try {
    ...
    form.append("file", fs$1.createReadStream(filePath));
...
const iconPath = process.env.VITE_DEV_SERVER_URL ? path$1.join(process.cwd(), "public/icon.png") : path$1.join(__dirname, "../../dist/icon.png");
function startPythonEngine() {
  var _a, _b;
  if (pythonProcess && !pythonProcess.killed) return;
  const isDev = !!process.env.VITE_DEV_SERVER_URL;
  const rootDir = process.cwd();
  const pythonExe = isDev ? path$1.join(rootDir, "python_engine", "venv", "Scripts", "python.exe") : path$1.join(process.resourcesPath, "python_engine", "python.exe");
  const mainPy = isDev ? path$1.join(rootDir, "python_engine", "main.py") : path$1.join(process.resourcesPath, "python_engine", "main.py");
  if (!fs$1.existsSync(pythonExe) || !fs$1.existsSync(mainPy)) return;
  pythonProcess = child_process.spawn(
    ...
    {
      cwd: path$1.dirname(mainPy),
      windowsHide: true
    }
  );
  (_a = pythonProcess.stdout) == null ? void 0 : _a.on("data", (d) => {
    if (d.toString().trim()) logSystem("INFO", `[PY]: ${d.toString().trim()}`);
  });
  (_b = pythonProcess.stderr) == null ? void 0 : _b.on("data", (d) => {
...
function killPythonByType() {
  if (pythonProcess) {
    if (process.platform === "win32") {
      try {
        child_process.exec(`taskkill /F /T /PID ${pythonProcess.pid}`);
      } catch (e) {
      }
    } else {
      pythonProcess.kill();
...
function getPreloadPath() {
  ...
  return fs$1.existsSync(p) ? p : path$1.resolve(
    __dirname,
...
function createSplashWindow() {
  ...
  const splashFile = process.env.VITE_DEV_SERVER_URL ? path$1.join(process.cwd(), "public/splash.html") : path$1.join(__dirname, "../../dist/splash.html");
...
function setupApp() {
  ...
  setTimeout(() => {
    if (!initDB()) {
      ...
      electron.dialog.showErrorBox("Errore", "Impossibile caricare il database.");
      electron.app.quit();
    } else {
  ...
  electron.ipcMain.handle("open-log-file", () => openSystemLog());
  ...
  electron.ipcMain.handle("get-acquisti", () => getAcquisti());
  ...
  electron.ipcMain.handle(
    "analyze-pdf",
    async (e, p) => parseBankStatement(p, getMembri())
  );
}
function createMainWindow() {
  ...
  else win.loadFile(path$1.join(__dirname, "../../dist/index.html"));
